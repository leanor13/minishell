...
/*   Updated: 2024/06/10 15:09:27 by thuy-ngu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	find_quote(t_sign **lst, char *str, int i)
{
	...
	{
		printf("find_quote FIRST_DOUBLE_QUOTE\n");
		if (str[i] == 34)
		{
	...
	{
		printf("find_quote FIRST_SINGLE_QUOTE\n");
		if (str[i] == 39)
		{
			printf("ITTVAN\n");
...
int	handle_quotestring(t_arg **lst, t_sign **quote, char *str, int i)
{
	...
	{
		printf("FIRST: handle_quotestring FIRST_DOUBLE_QUOTE\n");
	...
	{
		printf("FIRST: handle_quotestring FIRST_SINGLE_QUOTE \n");
	...
	{
		printf("FIRST: handle_quotestring SECOND_SINGLE_QUOTE\n");
		ft_bzero(*quote, 1);
		return (1);
	}
	if ((*quote)->quote_type == SECOND_DOUBLE_QUOTE)
	{
		printf("FIRST: handle_quotestring  SECOND_DOUBLE_QUOTE\n");
		ft_bzero(*quote, 1);
		return (1);
	}
	ft_bzero(*quote, 1);
	...
	(str[i] == 39 && str[i + 1] == 39))
	{
		printf("FIRST: handle_quotestring return empty\n");
		(*quote)->quote_type = 0;
		return (2);
	}
	if (str[i] == 34)
	{
		printf("FIRST: handle_quotestring 34 FIRST_DOUBLE_QUOTE\n");
		(*quote)->quote_type = FIRST_DOUBLE_QUOTE;
		return (1);
	}
	if (str[i] == 39)
	{
		printf("FIRST: handle_quotestring 39 FIRST_SINGLE_QUOTE\n");
